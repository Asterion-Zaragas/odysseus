...
_register(
    {
        ...
        "vault_get",
        "vault_search",
        # Read-only recall over the user's saved memories. Same profile as
        # `manage_memory` search/list and `search_chats`: the store is private,
        # and its contents are distilled from conversations, so a memory can
        # carry text that originally arrived from an untrusted source.
        # Unregistered it would fall through to _UNKNOWN_CAPABILITIES, which
        # fails high and claims every effect (execute_code, admin_change,
        # destructive, …) — far more than this tool can actually do.
