...
function _buildModelRow(mid, url, displayName, endpointId, offline, modelType, filename) {
  ...
  span.className = 'grow models-row-name';
  const primaryName = displayName.split('/').pop();
  const primary = document.createElement('span');
  primary.className = 'models-row-name-primary';
  primary.textContent = primaryName;
  span.appendChild(primary);
  // Friendly name (Cookbook launch / Admin panel) is the primary label;
  // show the raw filename as a dim subtitle underneath when it's set and
  // differs from the friendly name — otherwise there's nothing to add.
  if (filename && filename !== primaryName) {
    const sub = document.createElement('span');
    sub.className = 'models-row-name-sub';
  ...
  try {
    ...
    if (_cachedItems && _cachedItems.length > 0) {
      _cachedItems.forEach(item => {
        ...
        const filenames = item.models_filename || item.models || [];
        ...
        const extraFilenames = item.models_extra_filename || item.models_extra || [];
        (item.models_extra || []).forEach((mid, i) => {
          extraGroups[cat][epName].push({
            mid, url: item.url,
            displayName: extraDisplayNames[i] || mid,
            filename: extraFilenames[i] || mid,
            endpointId: item.endpoint_id || null,
            offline: isOffline,
            modelType: epModelType,
          });
    ...
    if (favs.length > 0) {
      ...
      if (favModels.length > 0) {
        ...
        if (!favCollapsed) {
          ...
          favModels.forEach(({ mid, url, displayName, endpointId, offline, modelType, filename }) => {
            favContainer.appendChild(_buildModelRow(mid, url, displayName, endpointId, offline, modelType, filename));
          });
    ...
    categoryOrder.forEach(({ key, label }) => {
      ...
      Object.entries(endpoints).forEach(([epName, epModels]) => {
        ...
        visible.forEach(({ mid, url, displayName, endpointId, offline, modelType, filename }) => {
          target.appendChild(_buildModelRow(mid, url, displayName, endpointId, offline, modelType, filename));
        });
        ...
        if (allHidden.length > 0) {
          ...
          showMoreBtn.addEventListener('click', () => {
            showMoreBtn.remove();
            allHidden.forEach(({ mid, url, displayName, endpointId, offline, modelType, filename }) => {
              target.appendChild(_buildModelRow(mid, url, displayName, endpointId, offline, modelType, filename));
            });
          });
    ...
    if (totalModelCount >= 10) {
      ...
      searchBox.addEventListener('input', () => {
        ...
        (_cachedItems || []).forEach(item => {
          ...
          const allFilenames = (item.models_filename || []).concat(item.models_extra_filename || item.models_extra || []);
          allModels.forEach((mid, i) => {
            const display = allDisplay[i] || mid;
            if (!mid.toLowerCase().includes(q) && !display.toLowerCase().includes(q)) return;
            searchResults.appendChild(
              _buildModelRow(mid, item.url, display, item.endpoint_id || null, false, item.model_type || 'llm', allFilenames[i] || mid)
            );
          });
        });
