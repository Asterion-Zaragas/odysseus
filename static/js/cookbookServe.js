...
function _rerenderCachedModels() {
  ...
  list.querySelectorAll('.memory-item[data-repo]').forEach(item => {
    item.addEventListener('click', async (e) => {
      ...
      panelHtml += `<label>${_l('Friendly name','Optional cosmetic name shown in the chat model picker instead of the raw model id/filename. The actual filename still shows underneath it. Leave blank to just show the filename.')}<input type="text" class="hwfit-sf hwfit-sf-wide" data-field="friendly_name" value="${esc(sv('friendly_name', ''))}" placeholder="${esc(shortName)}" /></label>`;
