...
def test_audit_calls_curator_for_the_caller_owner(monkeypatch):
    # The /audit route no longer resolves an endpoint/model itself — the
    # curator resolves its own "memory smart" role internally (falls back
    # through the same background-task chain as every other memory-system
    # agent), so this only needs to check the route forwards owner/dry_run
    ...
    async def fake_curate(memory_manager_arg, memory_vector_arg, owner=None, dry_run=False, interactive=False, force=False):
        curate_calls.append((memory_manager_arg, memory_vector_arg, owner, dry_run, interactive, force))
        return {"before": 2, "after": 1, "already_tidy": False}

    monkeypatch.setattr(mr, "curate", fake_curate)

    out = asyncio.run(audit_route(request=_request("alice"), dry_run=True, force=False))
    ...
    assert curate_calls == [(memory_manager, memory_vector, "alice", True, True, False)]
