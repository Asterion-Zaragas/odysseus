...
async def test_failed_url_prefetch_keeps_diagnostics_out_of_model_context(monkeypatch):
    monkeypatch.setattr(
        chat_processor,
    ...
    preface, _, _ = await _processor().build_context_preface(
        message=(
            "Please summarize "
...
async def test_failed_url_prefetch_exposes_only_stable_http_status(monkeypatch):
    monkeypatch.setattr(
        chat_processor,
        "fetch_webpage_content",
        lambda url: {
...
async def test_failed_url_prefetch_exposes_only_stable_failure_category(
...
async def test_unexpected_url_prefetch_exception_does_not_abort_context(monkeypatch):
    def fail(url):
...
async def test_unexpected_url_prefetch_exception_logs_only_stable_diagnostic(
    monkeypatch, caplog
):
    def fail(url):
    ...
    with caplog.at_level(logging.WARNING, logger=chat_processor.logger.name):
        await _processor().build_context_preface(
            message=(
...
async def test_successful_url_prefetch_keeps_existing_content_shape(monkeypatch):
    monkeypatch.setattr(
        chat_processor,
        "fetch_webpage_content",
